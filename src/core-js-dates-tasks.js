...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const countOfMs =
    new Date(Date.parse(dateEnd)).getTime() -
    new Date(Date.parse(dateStart)).getTime();
  const msInDay = new Date(0, 0, 1).getTime() - new Date(0, 0, 0).getTime();
  return 1 + Math.floor(countOfMs / msInDay);
}
...
function isDateInPeriod(date, period) {
  const start = new Date(Date.parse(period.start));
  const end = new Date(Date.parse(period.end));
  const current = new Date(Date.parse(date));
...
function getWeekNumberByDate(date) {
  const msInDay = new Date(0, 0, 2).getTime() - new Date(0, 0, 1).getTime();
  const firstDayOfYear = new Date(date.getFullYear(), 0, 1);
  const days =
    Math.floor((date.getTime() - firstDayOfYear.getTime()) / msInDay) + 1;
  const daysInFirstWeek =
    firstDayOfYear.getDay() !== 0 ? 8 - firstDayOfYear.getDay() : 1;
  if (days <= daysInFirstWeek) {
    return 1;
  }
  return 1 + Math.ceil((days - daysInFirstWeek) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  while (
    new Intl.DateTimeFormat('en-us', {
      weekday: 'long',
    }).format(friday13) !== 'Friday'
  ) {
    let month = friday13.getMonth() + 1;
    let year = friday13.getFullYear();
    if (month === 12) {
      month = 0;
      year += 1;
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  return [...Array(days + 1).keys()]
    .filter((x) => x % (countWorkDays + countOffDays) < countWorkDays)
    .map((date) =>
      new Intl.DateTimeFormat('fr-CA', {
        ...
        .format(new Date(msInDay * date + startDate + 1))
        .split('-')
        .reverse()
        .join('-')
    );
